    ...
    <div className="min-h-screen text-white relative overflow-x-hidden scroll-smooth">
      ...
      <div className="fixed top-0 left-0 right-0 z-40 flex justify-center gap-4 p-4 backdrop-blur-xl bg-white/5 border-b border-white/10">
        <button
          ...
          className={`px-4 py-2 rounded-full border transition hover:scale-105 ${activeTab === "watch" ? "bg-white text-black" : "border-white/20"}`}
        >
          Watch
        </button>
        <button
          onClick={() => { playClick(); setActiveTab("photo"); }}
          className={`px-4 py-2 rounded-full border transition hover:scale-105 ${activeTab === "photo" ? "bg-white text-black" : "border-white/20"}`}
        >
          Experience
        </button>
        <button
          onClick={() => { playClick(); setActiveTab("game"); }}
          className={`px-4 py-2 rounded-full border transition hover:scale-105 ${activeTab === "game" ? "bg-white text-black" : "border-white/20"}`}
        >
          Games
        </button>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {galleryPhotos.map((img, i) => (
              <img
                key={i}
                src={img}
                onClick={() => { playClick(); setPreview(img); }}
