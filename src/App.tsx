...
function FlappyCanvasGame() {
  ...
  const gameState = useRef({
    ...
    score: 0, highScore: parseInt(localStorage.getItem("flappyHigh") || "0"),
    gameOver: false, started: false, speed: 2.5,
  });
  ...
  const init = useCallback(() => {
    ...
    s.catY = 250; s.catVel = 0; s.pipes = []; s.score = 0; s.gameOver = false; s.started = false; s.speed = 2.5;
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full relative" onClick={tap} onTouchStart={(e) => { e.preventDefault(); tap(); }}>
...
function AvoidGame() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const gameState = useRef({
    catX: 150, balls: [] as { x: number; y: number; r: number }[],
    score: 0, highScore: parseInt(localStorage.getItem("avoidHigh") || "0"),
  ...
  const init = useCallback(() => {
    const s = gameState.current;
    s.catX = 150; s.balls = []; s.score = 0; s.gameOver = false; s.started = false;
    setScore(0); setGameOver(false); setStarted(false);
  }, []);
  ...
  const loop = useCallback(() => {
    ...
    if (!s.started || s.gameOver) {
      ...
      ctx.fillText(s.gameOver ? "Game Over" : "Geser kucing untuk mulai", w / 2, h / 2 - 20);
      ctx.fillText(`Skor: ${s.score}   High: ${s.highScore}`, w / 2, h / 2 + 20);
      animRef.current = requestAnimationFrame(loop); return;
    }
    ...
    ctx.font = "36px Arial"; ctx.fillText("🐱", s.catX - 18, h - 20);
    ctx.fillStyle = "white"; ctx.font = "bold 20px sans-serif"; ctx.textAlign = "center"; ctx.fillText(String(s.score), w / 2, 30);
    animRef.current = requestAnimationFrame(loop);
  }, []);
  ...
  const move = (clientX: number) => {
    ...
    gameState.current.catX = Math.max(30, Math.min(clientX - rect.left, rect.width - 30));
    if (!gameState.current.started) { gameState.current.started = true; setStarted(true); }
  };
...
function MemoryGame() {
  ...
  const restart = () => {
    ...
    setOpened([]); setMoves(0); setLocked(false);
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full max-w-md mx-auto gap-4">
      ...
      <div className="flex justify-between w-full text-sm text-white/60">
        ...
        <button onClick={restart} className="text-cyan-400">Ulangi</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen text-white relative overflow-x-hidden pb-24" style={{ background: getBackground(), backgroundAttachment: "fixed", userSelect: "none", WebkitTouchCallout: "none", cursor: "none" }}>
      ...
      <style>{`
        ...
        @keyframes borderShimmer { 0% { border-color: rgba(0,255,200,0.3); box-shadow: 0 0 12px rgba(0,255,200,0.2); } 50% { border-color: rgba(0,255,200,0.8); box-shadow: 0 0 24px rgba(0,255,200,0.4); } 100% { border-color: rgba(0,255,200,0.3); box-shadow: 0 0 12px rgba(0,255,200,0.2); } }
        @keyframes rgbBorder { 0% { border-color: #00ffcc; box-shadow: 0 0 15px #00ffcc; } 33% { border-color: #ccff00; box-shadow: 0 0 15px #ccff00; } 66% { border-color: #00ccff; box-shadow: 0 0 15px #00ccff; } 100% { border-color: #00ffcc; box-shadow: 0 0 15px #00ffcc; } }
        @keyframes avatarGlow { 0%, 100% { box-shadow: 0 0 20px rgba(0,255,200,0.4), 0 0 40px rgba(0,255,200,0.2); } 50% { box-shadow: 0 0 30px rgba(0,255,200,0.6), 0 0 60px rgba(0,255,200,0.3); } }
        ...
        @keyframes lightningFlash { 0% { opacity: 1; transform: scale(1); } 10% { opacity: 0.9; transform: scale(1.02); } 20% { opacity: 1; transform: scale(1); } 30% { opacity: 0.5; transform: scale(1.01); } 40% { opacity: 0.9; transform: scale(1); } 100% { opacity: 0; transform: scale(1); } }
        @keyframes tabFadeIn { from { opacity: 0; transform: translateY(10px) scale(0.98); } to { opacity: 1; transform: translateY(0) scale(1); } }
        ...
        .shimmer-text { background: linear-gradient(90deg, #34d399 0%, #facc15 40%, #ffffff 50%, #facc15 60%, #34d399 100%); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: shimmerText 3s linear infinite; }
        .shake { animation: shake 0.5s ease-out; }
        .lock-pulse { animation: lockPulse 2s ease-in-out infinite; }
        .rgb-border { border: 3px solid #00ffcc; animation: rgbBorder 4s linear infinite; transition: transform 0.3s ease; }
        .rgb-border:active { transform: scale(0.96); }
        .glass-card { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 1.5rem; box-shadow: 0 10px 30px -10px rgba(0,0,0,0.3); }
        .glass-nav { background: rgba(10, 30, 25, 0.7); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-top: 1px solid rgba(255,255,255,0.1); }
        .nav-btn-circle { width: 44px; height: 44px; border-radius: 50%; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 20px; cursor: pointer; background: rgba(0,255,200,0.1); border: 1px solid rgba(0,255,200,0.4); color: white; transition: background 0.2s, transform 0.15s; }
        .nav-btn-circle:disabled { color: rgba(255,255,255,0.2); cursor: default; }
        .nav-btn-circle:not(:disabled):active { transform: scale(0.9); }
        .social-card { transition: all 0.3s ease; border-radius: 1rem; }
        .social-card:hover { transform: scale(1.02); box-shadow: 0 0 20px rgba(0,255,200,0.4); }
        ...
        <>
          ...
          {preview && (
            <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.95)", backdropFilter: "blur(20px)", opacity: lightboxVisible ? 1 : 0, transition: "opacity 0.25s ease" }} onClick={closePreview} onContextMenu={(e) => e.preventDefault()}
              onTouchStart={(e) => { touchStartX.current = e.touches[0].clientX; }} onTouchEnd={(e) => { if (touchStartX.current === null) return; const diff = touchStartX.current - e.changedTouches[0].clientX; if (Math.abs(diff) > 50) diff > 0 ? swipeNext() : swipePrev(); touchStartX.current = null; }}>
              ...
              <div className="relative inline-block" onClick={(e) => e.stopPropagation()}>
                <img key={preview.index} src={preview.photos[preview.index]} className="anim-fadescale" style={{ maxWidth: "90%", maxHeight: "80vh", borderRadius: "16px", boxShadow: "0 25px 60px rgba(0,0,0,0.8)", userSelect: "none", WebkitTouchCallout: "none" }}
                  ...
                  onTouchEnd={(e) => { const now = Date.now(); if (now - parseInt((e.currentTarget as HTMLElement).dataset.lastTap || "0") < 300) { const key = `preview-${preview.photos[preview.index]}`; handleDoubleTap(e, key); } (e.currentTarget as HTMLElement).dataset.lastTap = String(now); }} />
